...
internal class SemiBlockingMultiMapTest {
    ...
    @Test
    fun `adding elements at roughly the same time does not interfere with each other`(testReporter: TestReporter) =
            testBlocking(
                    ForkJoinPool(10).asCoroutineDispatcher()) {
                val elementCount = 50
                // arrange
                ...
                repeat(elementCount) {
                    launch {
                        ...
                        testReporter.publishEntry(
                                "[${Thread.currentThread().name}] Just added value $it with a count of ")
                    }
                }
                delay(500)
    ...
    @Test
    fun `adding and removing elements at roughly the same time does not interfere with each other`(testReporter: TestReporter) =
            testBlocking(
                    ForkJoinPool(10).asCoroutineDispatcher()) {
                val addCount = 50
